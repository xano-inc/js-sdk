...
var XanoRealtimeChannel = /** @class */ (function () {
    function XanoRealtimeChannel(channel, options, config) {
        ...
        this.socketObserver = realtime_state_1.XanoRealtimeState.getInstance().getSocketObserver();
        this.onFuncs = [];
            ...
            XanoRealtimeObserver.prototype.update = function (rawAction) {
                ...
                switch (action.action) {
                    ...
                    case realtime_action_1.ERealtimeAction.Join:
                        ...
                        break;
                    case realtime_action_1.ERealtimeAction.Error:
                ...
                for (var _i = 0, _c = this.realtimeChannel.onFuncs; _i < _c.length; _i++) {
                    ...
                    if (action.action === realtime_action_1.ERealtimeAction.Error) {
                        ...
                        var returned = onFunc.onFunc(action);
        ...
        var state = realtime_state_1.XanoRealtimeState.getInstance().setConfig(this.config);
    ...
    XanoRealtimeChannel.prototype.sendJoin = function (socket, message, attempt) {
        ...
        if (current.readyState !== WebSocket.OPEN) {
            this.pendingJoin = { message: message, attempt: attempt };
            setTimeout(function () { return _this.retryJoinIfUnacknowledged(socket, message, attempt); }, XanoRealtimeChannel.JOIN_RETRY_MS);
            return;
        }
     ...
     */
    XanoRealtimeChannel.prototype.handleJoinRefused = function (action) {
        var _a;
        var pending = this.pendingJoin;
        if (pending === null || this.joinAcknowledged) {
            return;
        }
        var message = (_a = action === null || action === void 0 ? void 0 : action.payload) === null || _a === void 0 ? void 0 : _a.message;
    ...
    XanoRealtimeChannel.prototype.retryJoinIfUnacknowledged = function (socket, message, attempt) {
    ...
    XanoRealtimeChannel.prototype.handlePresenceUpdate = function (action) {
        ...
        if (action.action === realtime_action_1.ERealtimeAction.PresenceLeave) {
            ...
            var index = this.presenceCache.findIndex(function (item) {
                return _this.isSameMember(item, gone_1);
            });
            if (index !== -1) {
    ...
    XanoRealtimeChannel.prototype.message = function (payload, actionOptions) {
        ...
        var state = realtime_state_1.XanoRealtimeState.getInstance();
        if (!state.isV2()) {
            var message_1 = (0, realtime_build_action_util_1.realtimeBuildActionUtil)(realtime_action_1.ERealtimeAction.Message, __assign(__assign({}, actionOptions), { channel: this.channel }), payload);
            this.sendOrQueue(socket, message_1);
            return;
        }
        ...
        var perCallType = actionOptions.type, restOptions = __rest(actionOptions, ["type"]);
        var messageType = perCallType !== null && perCallType !== void 0 ? perCallType : this.options.messageType;
        ...
        if (!messageType) {
            throw new Error("Realtime v2 channel \"".concat(this.channel, "\" requires a message type. Set `messageType` on the channel, or pass `{ type }` to message()."));
        }
        var message = (0, realtime_build_action_util_1.realtimeBuildActionUtil)(realtime_action_1.ERealtimeAction.Broadcast, __assign(__assign({}, restOptions), { channel: this.channel }), payload, messageType);
        this.sendOrQueue(socket, message);
    };
    XanoRealtimeChannel.prototype.sendOrQueue = function (socket, message) {
        if (socket.readyState === WebSocket.OPEN) {
            socket.send(message);
        }
     ...
     */
    XanoRealtimeChannel.prototype.autoAck = function (action, pending) {
        var _this = this;
        ...
        // saw its own error.
        Promise.all(pending).then(function () { return _this.ack(cursor); }, function () { return undefined; });
    };
    /**
    ...
    XanoRealtimeChannel.prototype.ack = function (cursor) {
        ...
        socket.send((0, realtime_build_action_util_1.realtimeBuildActionUtil)(realtime_action_1.ERealtimeAction.Ack, { channel: this.channel, client_id: state.getClientId(), id: cursor }, { cursor: cursor }));
    };
     ...
     */
    XanoRealtimeChannel.JOIN_NOT_READY = "Connection is not ready";
